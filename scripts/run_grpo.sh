...
trainer = GRPOTrainer(policy_model, reference_model, reward_model, tokenizer, config['grpo'], device)
output_dir = Path(config['output_dir']) / 'grpo_model'
trainer.train(prompts, config['grpo']['num_epochs'], output_dir)
"
